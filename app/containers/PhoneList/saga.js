...
function* fetch({ payload }) {
  try {
    const response = yield call(API.fetchPhones, payload);
    yield put({
...
function* create({ payload }) {
  try {
    const response = yield call(API.createPhone, payload);
    const data = yield select(state => state.getIn(['phones', 'data']));
    yield put({
...
function* remove({ payload }) {
  try {
    yield call(API.deletePhone, payload);
    const data = yield select(state => state.getIn(['phones', 'data']));
    yield put({
      type: types.CREATE_SUCCESS,
